...
(function() {
  ...
  exports = module.exports = Storage = (function() {
    ...
    Storage.prototype.path = function(style) {
      var addToPaths, att, paths;
      att = this.attachedFile;
      paths = ['/'];
      addToPaths = function(object) {
        if (object != null) {
          return paths.push(object.toString());
        }
      };
      addToPaths(att != null ? att.modelName : void 0);
      addToPaths(att != null ? att.modelId : void 0);
      addToPaths(att != null ? att.attributeName : void 0);
      paths.push("" + att.id + "_" + style + att.extension);
      return path.join.apply(path, paths);
    };

    Storage.prototype.url = function(style) {
      return "" + Storage.baseUrl + (this.path(style));
    };
    ...
    Storage.prototype.flushDeletes = function(cb) {
      var deletes, store, style;
      store = this;
      deletes = (function() {
        var _i, _len, _ref, _results,
          _this = this;
        _ref = store.pendingDeletes;
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          style = _ref[_i].style;
          _results.push((function(style) {
            return function(done) {
              return store["delete"](style, done);
            };
          })(style));
        }
        return _results;
      }).call(this);
      return async.parallel(deletes, function(err) {
        if (err != null) {
          return cb(err);
        }
